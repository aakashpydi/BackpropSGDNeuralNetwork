...
class NeuralNetwork:
    ...
    def backward(self, target_output):
        ...
        self.total_error_neural_network = (1.0/2.0)*(target_output - self.actual_sigmoid_output[self.layer_count - 1]).pow(2).sum()  #(1/2)*(target_output - actual_output) ^ 2
        #print "TOTAL ERROR NEURAL NETWORK\t" + str(total_error_neural_network)
        ...
        for i in range(self.layer_count-2, -1, -1):
            ...
            if i == self.layer_count-2:
                self.output_mult_error.update({i: torch.mm(self.actual_sigmoid_output[i], error.t())})
                # print "multiplying"
                # print i
                ...
                error = torch.index_select(error, 0, torch.LongTensor([1, 2]))
                self.output_mult_error.update({i:torch.mm(self.actual_sigmoid_output[i], error.t())})

            derivative_current_layer = find_sig_derivative(self.actual_sigmoid_output[i])
            #print "DEBUG:\t" + str(self.edges_keys)
            #print str(i)+"\tdebug:\t" +str(self.theta[self.edges_keys[i]])
            #print str(i)+"\tdebug:\t" +str(error)
    ...
    def updateParams(self, learning_rate):
        ...
        for i in range(self.layer_count - 1):
                ...
                self.theta[self.edges_keys[i]] = self.theta[self.edges_keys[i]] - temp
